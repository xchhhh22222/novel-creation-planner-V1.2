...
description: V1.4 局部Scale小说创造规划器：沿用V1.3单书选项与素材来源硬门；用户选定后先完成前100章局部世界闭环（势力关系、2-3套体系、当前境界、功法/武技/法宝、普通资源、本地地图和金手指接口），通过SCALE_GATE后才能设计战略目标物与两个大高潮；每个高潮必须明确实力、核心收获、身份地位与权限的状态跃迁。
---
...
- **用户选定后，需要补当前100章局部世界：必须读取 [references/local-scale-100.md](references/local-scale-100.md)，通过 SCALE_GATE 后才能进入高潮。**
...
9. **构造前100章局部 Scale**：只规划当前城市/区域会用到的4—7个势力关系、0—2个外部势力接口、2—3套体系、当前境界、功法/武技/法宝、3—6种普通资源、3—6个本地地图节点和金手指接口。
...
- 选定方案的金手指必须有名称、输入、处理、输出、限制、代价、失败状态、克制方式和阶段成长；前100章 local scale 必须包含4—7个 active factions 的关系闭环、2—3套当前体系、当前境界、至少3个功法、3个武技、2个法宝/装备、3—6种普通资源、3—6个本地地图节点和金手指接口。
